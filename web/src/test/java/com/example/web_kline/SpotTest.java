...
import org.example.Position.SameQuantifyPositionManagementStrategy;
...
import org.example.binance.kline.UmFutureWsClient;
...
public class SpotTest {
    public static void main(String[] args) {
        ...
        UMWebsocketClientImpl client = BinanceClientFactory.UMFuturesClientImpl();
        //获取市场深度数据 买单的数量 减去 卖单的数量 * 价格 超过多少时，就代表可以买入了
        client.partialDepthStream("btcusdt", 20,500, response -> {
            System.out.println(JsonUtil.toJson(response));
        });


//        PositionManagerRepository positionManagerRepository = new PositionManagerRepository("atr_01", ContractType.SPOT, Exchange.BINANCE, OrderSide.BUY_LONG);
//        SameQuantifyPositionManagementStrategy sameQuantifyPositionManagementStrategy = new SameQuantifyPositionManagementStrategy(100, 1, positionManagerRepository);
//
//        Currency btcusdt = BinanceCurrencyRegister.getCurrency("BTCUSDT").get();
//
//        AtrStrateFactory atrStrateFactory = new AtrStrateFactory(positionManagerRepository, btcusdt);
//
//        AtrSpot atrSpot = new AtrSpot(btcusdt, "5m", sameQuantifyPositionManagementStrategy, atrStrateFactory);
//        atrSpot.trade();
    }


    public static StrateFactory strateFactory = new StrateFactory() {
        @Override
